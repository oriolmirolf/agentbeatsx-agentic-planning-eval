...
Per the evaluation framework, the Oracle tracks and reports costs using renamed fields for diagnostic clarity:

* **`input_tokens`**: Total tokens consumed by the Purple Agent during state queries and prompt processing.
* **`output_tokens`**: Total tokens generated by the Purple Agent for reasoning and tool calls.
